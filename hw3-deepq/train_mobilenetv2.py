...
def get_eval_spec():
    ...
    """
    transform = lambda image: tf.image.resize_images(image, [256, 256])
...
def before_batch(train_history, validation_history):
    ...
    """
    global lr
    return {'feed_dict': {lr: 0.2}, 'batch_norm': 0.9, 'drop_out': None}
best=0.85
def save_model_as(train_history, validation_history):
    ...
    """
    n_epoch = len(train_history)
    if(best<validation_history[n_epoch-1][0]):
        return 'save_dir'+str(n_epoch)
    else:
        return None
    """
    if n_epoch == 50:
